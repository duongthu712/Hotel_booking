...
Purpose: Display the homepage UI, including the room search
         banner, news list, featured services, and hotel gallery. --%>
...
<html lang="vi">
    ...
    <body>
            ...
            <form class="search-bar" action="${pageContext.request.contextPath}/search" method="GET">
                ...
                <select name="roomTypeId">
                    <option value="all" <c:if
                                test="${empty param.roomTypeId || param.roomTypeId eq 'all'}">selected</c:if>>
                                Tất cả loại phòng
                            </option>
                    <c:forEach var="item" items="${allRoomTypesList}">
                        <option value="${item.roomTypeId}" <c:if
                                    test="${param.roomTypeId eq item.roomTypeId.toString()}">selected</c:if>>
                                ${item.typeName}
                        </option>
                    </c:forEach>
                </select>
        ...
        <main class="container">
            ...
            <!-- Display Hotel Services & Amenities -->
            <section class="section">
                            ...
                            <c:forEach var="service" items="${services}">
                                <div class="card">
                                    ...
                                    <span class="price">
                                        <c:choose>
                                            <c:when
                                                test="${empty service.unitPrice || service.unitPrice < 1}">
                                                Miễn phí
                                            </c:when>
                                            <c:otherwise>
                                                <fmt:formatNumber value="${service.unitPrice}"
                                                                  type="number" /> ₫
                                            </c:otherwise>
                                        </c:choose>
                                    </span>
        ...
        <!-- Load booking calendar logic script -->
        <script
        src="${pageContext.request.contextPath}/view/assets/javascript/booking-calendar.js"></script>
    </body>
